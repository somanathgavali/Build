...
var s = document.querySelectorAll("script[src='https://accounts.google.com/gsi/client']");
if (s.length == 0) {
	...
	s.onload = function() {
		Module["GoogleIdentityServicesWebGL"] = Module["GoogleIdentityServicesWebGL"] || {};
		if (Module.GoogleIdentityServicesWebGL.onGoogleLibraryLoad) {//if onGoogleLibraryLoad already exists
			Module.dynCall_v(Module.GoogleIdentityServicesWebGL.onGoogleLibraryLoad);
		}
		else {
			Object.defineProperty(Module.GoogleIdentityServicesWebGL, "onGoogleLibraryLoad", {
				configurable: true,
				set: function(v){
					Object.defineProperty(Module.GoogleIdentityServicesWebGL, "onGoogleLibraryLoad", {configurable: true, enumerable: true, writable: true, value: v });
					Module.dynCall_v(Module.GoogleIdentityServicesWebGL.onGoogleLibraryLoad);
				}
			});
		}
...
function _cancel_GoogleIdentityServicesWebGL() {
...
function _disableAutoSelect_GoogleIdentityServicesWebGL() {
...
function _hideButton_GoogleIdentityServicesWebGL() {
 const div = document.getElementById("ButtonParent_GoogleIdentityServicesWebGL");
 if (!div) return;
 div.style.display = "none";
}

function _initialize_GoogleIdentityServicesWebGL(client_id, auto_select, callback, login_uri, native_callback, cancel_on_tap_outside, prompt_parent_id, nonce, context, state_cookie_domain, ux_mode, allowed_parent_origin, length, intermediate_iframe_close_callback) {
 const IdConfiguration = {};
 client_id = UTF8ToString(client_id);
 IdConfiguration.client_id = client_id;
 IdConfiguration.auto_select = auto_select;
 IdConfiguration.callback = function(CredentialResponse) {
  const credentialLength = lengthBytesUTF8(CredentialResponse.credential) + 1;
  const credentialPointer = Module._malloc(credentialLength);
  stringToUTF8(CredentialResponse.credential, credentialPointer, credentialLength);
  const selectbyLength = lengthBytesUTF8(CredentialResponse.select_by) + 1;
  const selectbyPointer = Module._malloc(selectbyLength);
  stringToUTF8(CredentialResponse.select_by, selectbyPointer, selectbyLength);
  Module.dynCall_vii(callback, credentialPointer, selectbyPointer);
 };
 if (login_uri !== 0) {
  login_uri = UTF8ToString(login_uri);
  IdConfiguration.login_uri = login_uri;
 }
 if (native_callback !== 0) {
  IdConfiguration.native_callback = function(CredentialResponse) {
   const credentialLength = lengthBytesUTF8(CredentialResponse.credential) + 1;
   const credentialPointer = Module._malloc(credentialLength);
   stringToUTF8(CredentialResponse.credential, credentialPointer, credentialLength);
   const selectbyLength = lengthBytesUTF8(CredentialResponse.select_by) + 1;
   const selectbyPointer = Module._malloc(selectbyLength);
   stringToUTF8(CredentialResponse.select_by, selectbyPointer, selectbyLength);
   Module.dynCall_vii(native_callback, credentialPointer, selectbyPointer);
  };
 }
 IdConfiguration.cancel_on_tap_outside = cancel_on_tap_outside;
 if (prompt_parent_id !== 0) {
  prompt_parent_id = UTF8ToString(prompt_parent_id);
  IdConfiguration.prompt_parent_id = prompt_parent_id;
 }
 if (nonce !== 0) {
  nonce = UTF8ToString(nonce);
  IdConfiguration.nonce = nonce;
 }
 if (context !== 0) {
  context = UTF8ToString(context);
  IdConfiguration.context = context;
 }
 if (state_cookie_domain !== 0) {
  state_cookie_domain = UTF8ToString(state_cookie_domain);
  IdConfiguration.state_cookie_domain = state_cookie_domain;
 }
 IdConfiguration.ux_mode = UTF8ToString(ux_mode);
 if (length !== 0) {
  const allowedParentOriginArr = [];
  for (var i = 0; i < length; i++) allowedParentOriginArr.push(UTF8ToString(HEAP32[allowed_parent_origin + i * 4 >> 2]));
  IdConfiguration.allowed_parent_origin = allowedParentOriginArr;
 }
 if (intermediate_iframe_close_callback !== 0) {
  IdConfiguration.intermediate_iframe_close_callback = function() {
   Module.dynCall_v(intermediate_iframe_close_callback);
  };
 }
...
function _onGoogleLibraryLoad_GoogleIdentityServicesWebGL(callback) {
 Module["GoogleIdentityServicesWebGL"] = Module["GoogleIdentityServicesWebGL"] || {};
 Module["GoogleIdentityServicesWebGL"].onGoogleLibraryLoad = callback;
}

function _prompt_GoogleIdentityServicesWebGL(callback) {
 if (callback !== 0) {
  google.accounts.id.prompt(function(notification) {
   const notDisplayedReasonLength = lengthBytesUTF8(notification.getNotDisplayedReason() || "") + 1;
   const notDisplayedReasonPointer = Module._malloc(notDisplayedReasonLength);
   stringToUTF8(notification.getNotDisplayedReason() || "", notDisplayedReasonPointer, notDisplayedReasonLength);
   const skippedReasonLength = lengthBytesUTF8(notification.getSkippedReason() || "") + 1;
   const skippedReasonPointer = Module._malloc(skippedReasonLength);
   stringToUTF8(notification.getSkippedReason() || "", skippedReasonPointer, skippedReasonLength);
   const dismissedReasonLength = lengthBytesUTF8(notification.getDismissedReason() || "") + 1;
   const dismissedReasonPointer = Module._malloc(dismissedReasonLength);
   stringToUTF8(notification.getDismissedReason() || "", dismissedReasonPointer, dismissedReasonLength);
   const momentTypeLength = lengthBytesUTF8(notification.getMomentType() || "") + 1;
   const momentTypePointer = Module._malloc(momentTypeLength);
   stringToUTF8(notification.getMomentType() || "", momentTypePointer, momentTypeLength);
   Module.dynCall_viiiiiiiii(callback, notification.isDisplayMoment(), notification.isDisplayed(), notification.isNotDisplayed(), notDisplayedReasonPointer, notification.isSkippedMoment(), skippedReasonPointer, notification.isDismissedMoment(), dismissedReasonPointer, momentTypePointer);
  });
 } else {
...
function _renderButton_GoogleIdentityServicesWebGL(width, height, x, y, type, theme, size, text, shape, logoAlignment, locale) {
 var div = document.getElementById("ButtonParent_GoogleIdentityServicesWebGL");
 if (!div) {
  const canvas = document.getElementsByTagName("canvas")[0];
  div = canvas.parentNode.appendChild(document.createElement("div"));
  div.setAttribute("id", "ButtonParent_GoogleIdentityServicesWebGL");
  div.style.left = x / window.devicePixelRatio + canvas.parentNode.getBoundingClientRect().width - canvas.getBoundingClientRect().width + "px";
  div.style.bottom = y / window.devicePixelRatio + canvas.parentNode.getBoundingClientRect().height - canvas.getBoundingClientRect().height + "px";
  div.style.position = "absolute";
 } else {
  if (div.hasChildNodes()) return;
 }
 const gsiButtonConfiguration = {};
 gsiButtonConfiguration.type = UTF8ToString(type);
 gsiButtonConfiguration.theme = UTF8ToString(theme);
 gsiButtonConfiguration.size = UTF8ToString(size);
 gsiButtonConfiguration.text = UTF8ToString(text);
 gsiButtonConfiguration.shape = UTF8ToString(shape);
 gsiButtonConfiguration.logo_alignment = UTF8ToString(logoAlignment);
 gsiButtonConfiguration.width = Math.round(width / window.devicePixelRatio).toString();
 if (locale !== 0) gsiButtonConfiguration.locale = UTF8ToString(locale);
 google.accounts.id.renderButton(div, gsiButtonConfiguration);
}

function _revoke_GoogleIdentityServicesWebGL(hint, callback) {
 google.accounts.id.revoke(UTF8ToString(hint), function(revocationResponse) {
  if (callback === 0) return;
  const errorLength = lengthBytesUTF8(revocationResponse.error || "") + 1;
  const errorPointer = Module._malloc(errorLength);
  stringToUTF8(revocationResponse.error || "", errorPointer, errorLength);
  Module.dynCall_vii(callback, revocationResponse.successful, errorPointer);
 });
}
...
function _showButton_GoogleIdentityServicesWebGL() {
 const div = document.getElementById("ButtonParent_GoogleIdentityServicesWebGL");
 if (!div) return;
...
function _storeCredential_GoogleIdentityServicesWebGL(id, password, callback) {
 const cred = {
  id: UTF8ToString(id),
  password: UTF8ToString(password)
 };
 if (callback !== 0) {
  google.accounts.id.storeCredential(cred, function() {
...
var asmLibraryArg = {
 ...
 "abort": _abort,
 "atexit": _atexit,
 "cancel_GoogleIdentityServicesWebGL": _cancel_GoogleIdentityServicesWebGL,
 "clock": _clock,
 "clock_getres": _clock_getres,
 "clock_gettime": _clock_gettime,
 "difftime": _difftime,
 "disableAutoSelect_GoogleIdentityServicesWebGL": _disableAutoSelect_GoogleIdentityServicesWebGL,
 "dlclose": _dlclose,
 "dlerror": _dlerror,
 "dlopen": _dlopen,
 "dlsym": _dlsym,
 ...
 "hideButton_GoogleIdentityServicesWebGL": _hideButton_GoogleIdentityServicesWebGL,
 "initialize_GoogleIdentityServicesWebGL": _initialize_GoogleIdentityServicesWebGL,
 ...
 "mktime": _mktime,
 "onGoogleLibraryLoad_GoogleIdentityServicesWebGL": _onGoogleLibraryLoad_GoogleIdentityServicesWebGL,
 "prompt_GoogleIdentityServicesWebGL": _prompt_GoogleIdentityServicesWebGL,
 "renderButton_GoogleIdentityServicesWebGL": _renderButton_GoogleIdentityServicesWebGL,
 "revoke_GoogleIdentityServicesWebGL": _revoke_GoogleIdentityServicesWebGL,
 "setTempRet0": _setTempRet0,
 "showButton_GoogleIdentityServicesWebGL": _showButton_GoogleIdentityServicesWebGL,
 "sigaction": _sigaction,
 "sigemptyset": _sigemptyset,
 "storeCredential_GoogleIdentityServicesWebGL": _storeCredential_GoogleIdentityServicesWebGL,
 "strftime": _strftime,
 "time": _time,
 "utime": _utime
};
